...
export function registerAuthTools(server: McpServer, ctx: ServerContext): void {
  server.registerTool(
    "m365_auth_status",
    {
      ...
      description:
        "Report the current authentication state: mode, signed-in account, token expiry, where the refresh token is stored, and which scopes were requested. Call this first when a Graph call returns 401 or 403.",
      inputSchema: {},
      annotations: { readOnlyHint: true },
    },
    async () =>
      runTool("m365_auth_status", async () => {
  ...
  server.registerTool(
    "m365_sign_in",
    {
      ...
      description:
        "Start the OAuth device-code flow and return the code and URL to give the user. Show them verbatim, wait for them to confirm, then retry the work. Optionally waits here for completion. The refresh token is stored in the OS keyring, so this is normally needed once per machine.",
      inputSchema: {
        waitSeconds: z
          .number()
          .int()
    ...
    async (args) =>
      runTool("m365_sign_in", async () => {
        const provider = ctx.auth;
        if (!(provider instanceof DeviceCodeAuthProvider)) {
          throw new M365Error(
            "AUTH_MODE",
            `Interactive sign-in does not apply in "${provider.mode}" mode.`,
            "Set OMATIC_M365_AUTH_MODE=device-code, or configure the app-only credential.",
          );
        }
  ...
  server.registerTool(
    "m365_sign_out",
    {
      ...
      description:
        "Drop the cached access token and delete the stored refresh token from the OS keyring. The next Graph call will require a new device-code sign-in.",
      inputSchema: {},
      annotations: { destructiveHint: true },
    },
    async () =>
      runTool("m365_sign_out", async () => {
