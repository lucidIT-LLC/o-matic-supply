...
import { DEFAULT_APP_ONLY_SCOPE } from "../config.js";
 ...
 */
export class ClientCredentialsAuthProvider implements IntrospectableAuthProvider {
  readonly mode = "client-credentials" as const;

  /** The scope an implementation must request. App-only ignores granular scopes. */
  static readonly scope = DEFAULT_APP_ONLY_SCOPE;
  ...
  get credentialAccount(): string {
    return `${this.config.tenantId}:${this.config.clientId}:client-credential`;
  }

  async getAccessToken(): Promise<string> {
    throw new NotImplementedError(
      "App-only (client credentials) authentication",
      `Set OMATIC_M365_AUTH_MODE=device-code to use the implemented delegated flow. ` +
        `See src/auth/client-credentials.ts for what implementing app-only requires.`,
    );
  }
  ...
  async status(): Promise<AuthStatus> {
    return {
      ...
      scopes: [ClientCredentialsAuthProvider.scope],
      detail:
        "App-only authentication is declared but not implemented. No Graph call will succeed in this mode.",
    };
  }

  async signOut(): Promise<void> {
    // App-only holds no user session. Removing the stored credential is a
    // deliberate, separate operator action, not a sign-out.
  }
}
