...
import {
  ...
  REFRESH_MARGIN_MS,
  type AuthStatus,
  type IntrospectableAuthProvider,
  type TokenSet,
  type TokenStore,
} from "./types.js";
 ...
 */
export class DeviceCodeAuthProvider implements IntrospectableAuthProvider {
  readonly mode = "device-code" as const;
  ...
  private pendingPrompt: DeviceCodePrompt | null = null;
  ...
  private get storeAccount(): string {
  ...
  private async acquireSilently(): Promise<TokenSet> {
    ...
    try {
      ...
      if (error instanceof InteractiveSignInRequired) throw error;
      throw error;
    }
  }

  private async redeemRefreshToken(refreshToken: string): Promise<TokenSet> {
    ...
    if (payload.error) {
      ...
      if (payload.error === "invalid_grant" || payload.error === "interaction_required") {
        ...
        throw new InteractiveSignInRequired(
          `the stored refresh token was rejected (${payload.error})`,
        );
      }
      throw new AuthError(
        `Refreshing the access token failed: ${payload.error}${payload.error_description ? ` — ${firstLine(payload.error_description)}` : ""}`,
      );
    }
   ...
   */
  async beginSignIn(): Promise<DeviceCodePrompt> {
    const body = new URLSearchParams({
      client_id: this.config.clientId,
      scope: this.config.scopes.join(" "),
    });
    ...
    const payload = (await safeJson(response)) as unknown as DeviceCodeResponse & TokenResponse;
    if (!response.ok || !payload.device_code) {
      throw new AuthError(
        `Requesting a device code failed (HTTP ${response.status})${payload.error ? `: ${payload.error}` : ""}${
          payload.error_description ? ` — ${firstLine(payload.error_description)}` : ""
        }`,
        `Confirm OMATIC_M365_TENANT_ID and OMATIC_M365_CLIENT_ID, and that the app registration has "Allow public client flows" enabled.`,
      );
    }
  ...
  async awaitSignIn(timeoutMs = 120_000): Promise<AuthStatus> {
    const pending = this.inFlight;
    if (!pending) {
      if (this.cached) return this.status();
      throw new AuthError(
        "No device-code sign-in is in progress.",
        "Call m365_sign_in first.",
      );
    }
    ...
    const timeout = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(
        () =>
          reject(
            new AuthError(
              `Still waiting for the user to complete sign-in after ${Math.round(timeoutMs / 1000)}s.`,
              `The device code is still valid — ask the user to finish, then call m365_auth_status.`,
            ),
          ),
        timeoutMs,
      );
  ...
  private async pollForToken(
    ...
    for (;;) {
      ...
      if (this.now() > expiresAtMs) {
        throw new AuthError(
          "The device code expired before sign-in was completed.",
          "Call m365_sign_in again to get a fresh code.",
        );
      }
      ...
      if (!payload.error) return this.adoptTokens(payload);
      ...
      switch (payload.error) {
        ...
        default: {
          ...
          throw new AuthError(
            known ??
              `Device-code sign-in failed: ${payload.error}${
                payload.error_description ? ` — ${firstLine(payload.error_description)}` : ""
              }`,
            "Call m365_sign_in again to restart the flow.",
          );
        }
      }
    }
  ...
  private async postForm(url: string, body: URLSearchParams): Promise<TokenResponse> {
    ...
    const payload = (await safeJson(response)) as unknown as TokenResponse;
  ...
  private async adoptTokens(payload: TokenResponse): Promise<TokenSet> {
    ...
    if (tokens.account) setAuditActor(tokens.account);
    log.info(
      `Access token acquired${tokens.account ? ` for ${tokens.account}` : ""}, expires ${new Date(tokens.expiresAt).toISOString()}.`,
    );
    return tokens;
  }
...
function accountFromIdToken(idToken: string | undefined): string | undefined {
  ...
  try {
    ...
    const claims = JSON.parse(json) as { preferred_username?: string; upn?: string; name?: string };
