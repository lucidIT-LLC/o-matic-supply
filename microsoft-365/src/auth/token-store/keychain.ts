...
export class KeychainTokenStore implements TokenStore {
  ...
  static assertSupported(): void {
    if (!KeychainTokenStore.isSupported()) {
      throw new TokenStoreError(
        `The macOS Keychain token store cannot run on platform "${platform()}".`,
        `Implement a TokenStore for this platform (see src/auth/token-store/) or run the server on macOS.`,
      );
    }
  }
  ...
  async get(account: string): Promise<string | null> {
    try {
      ...
      throw new TokenStoreError(
        `Reading the stored refresh token from the macOS Keychain failed (exit ${String(failure.code)}).`,
        `Check that the login keychain is unlocked, then retry. Nothing is cached elsewhere, so this is recoverable.`,
      );
    }
  }
  ...
  async set(account: string, secret: string): Promise<void> {
    try {
      ...
      throw new TokenStoreError(
        `Storing the refresh token in the macOS Keychain failed (exit ${String(failure.code)}).`,
        `Unlock the login keychain and retry. Until this succeeds every restart will require a fresh device-code sign-in.`,
      );
    }
  }
  ...
  async delete(account: string): Promise<void> {
    try {
      ...
      throw new TokenStoreError(
        `Deleting the stored refresh token from the macOS Keychain failed (exit ${String(failure.code)}).`,
        `Remove the "${this.service}" entry manually in Keychain Access if this persists.`,
      );
    }
  }
}
