...
export function registerTeamsTools(server: McpServer, ctx: ServerContext): void {
  const { teams } = ctx;
  ...
  server.registerTool(
    "teams_list_joined_teams",
    {
      ...
      description:
        "List the Teams the signed-in user is a member of. A Team's id is also the Microsoft 365 group id, so it can be passed to planner_list_plans as groupId.",
      inputSchema: {
    ...
    async (args) =>
      runTool("teams_list_joined_teams", async () => {
  ...
  server.registerTool(
    ...
    async (args) =>
      runTool("teams_list_channels", async () => {
  ...
  server.registerTool(
    "teams_list_channel_messages",
    {
      ...
      description:
        "Read recent messages in a channel. Read only — this server does not post to Teams. Requires the ChannelMessage.Read.All permission, which many tenants restrict.",
      inputSchema: {
    ...
    async (args) =>
      runTool("teams_list_channel_messages", async () => {
      ...
      runTool(
        ...
        async () => {
          if (args.confirm !== true) {
            throw new Error(
              "Refused: posting to a channel needs confirm=true. The message would be visible to everyone in the team.",
            );
          }
  ...
  server.registerTool(
    "teams_reply_to_message",
    {
      ...
      description:
        "Reply inside an existing channel message thread. Prefer this over teams_send_channel_message when continuing a conversation — a new top-level post fragments the thread. Requires confirm=true.",
      inputSchema: {
        teamId: z.string().describe("Team (group) ID."),
        channelId: z.string().describe("Channel ID."),
    ...
    async (args) =>
      runTool(
        ...
        async () => {
          ...
          const message = await teams.replyToChannelMessage(
            args.teamId,
            args.channelId,
            args.messageId,
            args.content,
