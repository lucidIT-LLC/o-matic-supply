 ...
 */
const LEGACY_ENV: Record<keyof typeof ENV, string> = {
...
interface FileConfig {
  ...
  graphBaseUrl?: unknown;
  tokenStoreService?: unknown;
}

function candidateFiles(env: NodeJS.ProcessEnv): string[] {
...
function loadFileConfig(env: NodeJS.ProcessEnv): { data: FileConfig; path: string | null } {
  for (const path of candidateFiles(env)) {
    ...
    try {
      ...
      return { data: parsed as FileConfig, path };
    } catch (error) {
      throw new ConfigError(
        `Config file ${path} could not be parsed: ${(error as Error).message}`,
        `Fix the JSON, or delete the file and use the ${ENV.tenantId} / ${ENV.clientId} environment variables instead.`,
      );
    }
  }
...
function missing(name: keyof typeof ENV, human: string, filePath: string | null): never {
  ...
  throw new ConfigError(
    `${human} is not configured. Team Assistant has no default for it and will not guess.`,
    [
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  if (!tenantId) missing("tenantId", "Entra directory (tenant) ID", file.path);
  if (!GUID.test(tenantId) && !tenantId.includes(".") && !TENANT_ALIASES.has(tenantId.toLowerCase())) {
    throw new ConfigError(
      `Tenant ID "${tenantId}" is neither a GUID, a verified domain, nor a known alias.`,
      `Use the "Directory (tenant) ID" GUID from the Entra app registration Overview blade.`,
    );
  }

  const clientId = pick("clientId", file.data.clientId);
  if (!clientId) missing("clientId", "Entra application (client) ID", file.path);
  if (!GUID.test(clientId)) {
    throw new ConfigError(
      `Client ID "${clientId}" is not a GUID.`,
      `Use the "Application (client) ID" from the Entra app registration Overview blade.`,
    );
  }
  ...
  if (authModeRaw !== "device-code" && authModeRaw !== "client-credentials") {
    throw new ConfigError(
      `Unknown auth mode "${authModeRaw}".`,
      `Set ${ENV.authMode} to "device-code" (delegated, acts as the signed-in user) or "client-credentials" (app-only).`,
    );
  }
  ...
  if (!scopesRaw) sources["scopes"] = "default";
  ...
  return {
    ...
    scopes,
    authorityHost: (pick("authorityHost", file.data.authorityHost, DEFAULT_AUTHORITY_HOST) as string).replace(/\/+$/, ""),
    graphBaseUrl: (pick("graphBaseUrl", file.data.graphBaseUrl, DEFAULT_GRAPH_BASE_URL) as string).replace(/\/+$/, ""),
    tokenStoreService: pick("tokenStoreService", file.data.tokenStoreService, DEFAULT_TOKEN_SERVICE) as string,
    sources,
  };
}
