...
describe("DeviceCodeAuthProvider — silent acquisition", () => {
  test("with nothing stored it asks for interactive sign-in rather than hanging", async () => {
    ...
    const error = await provider.getAccessToken().then(() => null).catch((e: unknown) => e);
...
describe("DeviceCodeAuthProvider — device code flow", () => {
  test("returns the user code, then polls through pending and slow_down to success", async () => {
    ...
    const fetchImpl = async (url: string, init?: RequestInit): Promise<Response> => {
      ...
      if (call === 2) return new Response(JSON.stringify({ error: "authorization_pending" }), { status: 400 });
      if (call === 3) return new Response(JSON.stringify({ error: "slow_down" }), { status: 400 });
      return new Response(
        JSON.stringify({
  ...
  test("a declined sign-in fails with a readable reason", async () => {
    ...
    const error = await provider.awaitSignIn(5000).then(() => null).catch((e: unknown) => e);
    assert.ok(error instanceof AuthError);
