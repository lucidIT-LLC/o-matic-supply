...
import {
  ...
  decodeReferenceKey,
  CHECKLIST_ITEM_TYPE,
} from "../src/planner/semantics.js";
import type { PlannerChecklist } from "../src/planner/types.js";
...
const current: PlannerChecklist = {
...
describe("checklist read-modify-write", () => {
  ...
  test("toggle flips the current value read from the fresh copy", () => {
    const { patch } = buildChecklistPatch(
      current,
      [{ action: "toggle", id: "item-a" }, { action: "toggle", id: "item-b" }],
      ids(),
    );
  ...
  test("sequential operations compose against the projected state", () => {
    const { patch, projected } = buildChecklistPatch(
      current,
      [
  ...
  test("an unknown item fails loudly and lists what is there", () => {
    assert.throws(
      () => buildChecklistPatch(current, [{ action: "check", title: "Nope" }], ids()),
      (error: unknown) =>
  ...
  test("an ambiguous title fails rather than picking one", () => {
    const ambiguous: PlannerChecklist = {
      one: { title: "Review", isChecked: false },
      two: { title: "review", isChecked: false },
    };
    assert.throws(
      () => buildChecklistPatch(ambiguous, [{ action: "check", title: "Review" }], ids()),
      (error: unknown) => error instanceof M365Error && error.code === "PLANNER_CHECKLIST_AMBIGUOUS",
    );
  });
