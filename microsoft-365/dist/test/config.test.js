...
describe("configuration", () => {
    ...
    test("complains about the client ID once the tenant is set", () => {
        assert.throws(() => loadConfig(env({ OMATIC_M365_TENANT_ID: TENANT })), (e) => e instanceof ConfigError && /TEAM_ASSISTANT_CLIENT_ID/.test(e.toToolMessage()));
    });
    test("rejects a client ID that is not a GUID rather than failing later at Entra", () => {
        assert.throws(() => loadConfig(env({ OMATIC_M365_TENANT_ID: TENANT, OMATIC_M365_CLIENT_ID: "my-app" })), ConfigError);
    });
