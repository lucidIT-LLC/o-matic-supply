...
import { makePlanner, stubFetch, FAKE_TOKEN } from "./helpers.js";
...
describe("Planner ETag handling", () => {
    ...
    test("a 412 on details names the details resource, not the task", async () => {
        const { fetchImpl } = stubFetch((request) => {
            if (request.method === "GET" && request.url === DETAILS_URL)
    ...
    test("the task ETag and the details ETag are never interchanged", async () => {
        const { fetchImpl, requests } = stubFetch((request) => {
            if (request.url === DETAILS_URL)
                return { body: detailsBody() };
            if (request.url === TASK_URL)
                return { body: taskBody(TASK_ETAG_V1) };
    ...
    test("a checklist delta is computed from the copy whose ETag is sent", async () => {
        const { fetchImpl, requests } = stubFetch((request) => {
            if (request.url === DETAILS_URL && request.method === "GET")
                return { body: detailsBody() };
            return { body: detailsBody() };
        });
        const planner = makePlanner(fetchImpl);
        ...
        const checklist = (patch?.body).checklist;
...
//# sourceMappingURL=etag.test.js.map