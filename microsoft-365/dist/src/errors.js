...
export class GraphError extends M365Error {
    ...
    constructor(detail, hint) {
        ...
        if (detail.graphCode)
            parts.push(`code=${detail.graphCode}`);
        if (detail.graphMessage)
            parts.push(`message=${detail.graphMessage}`);
        if (detail.requestId)
            parts.push(`request-id=${detail.requestId}`);
...
export class EtagConflictError extends M365Error {
    ...
    constructor(resource, detail) {
        super("PLANNER_ETAG_CONFLICT", `This ${resource} changed in Planner between the read and the write, so the update was rejected (HTTP 412) and nothing was saved.`, `Re-read the ${resource} to see its current state, decide whether your change still applies, then retry the write. Do not reuse the previous copy — the ETag it carried is dead.`);
        this.resource = resource;
        this.detail = detail;
    }
}
/** Convert anything thrown into a safe, caller-readable message. */
export function toToolMessage(error) {
