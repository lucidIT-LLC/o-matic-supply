...
import { buildChecklistPatch, buildReferencePatch, decodeReferenceKey, describeCategories, describePriority, describeProgress, } from "./semantics.js";
/**
 * Planner repository.
 *
 * ============================ ETag discipline ============================
...
export class PlannerApi {
    graph;
    users;
    constructor(graph, users) {
        this.graph = graph;
        this.users = users;
    }
    /* ------------------------------------------------------------------ plans */
    /** Plans in a specific group (Microsoft 365 group / Team). */
    async listPlansByGroup(groupId) {
        return this.graph.getAll(`/groups/${encodeURIComponent(groupId)}/planner/plans`);
    }
    /** Plans the signed-in user can see in their Planner hub. */
    async listMyPlans() {
        return this.graph.getAll("/me/planner/plans");
    }
    /** Unified groups the signed-in user belongs to — the containers of plans. */
    async listMyGroups() {
        return this.graph.getAll("/me/memberOf/microsoft.graph.group", { $select: ["id", "displayName", "mail", "groupTypes"], $top: 100 });
    }
    async getPlan(planId) {
        return this.graph.get(`/planner/plans/${encodeURIComponent(planId)}`);
    }
    async getPlanDetails(planId) {
        return this.graph.get(`/planner/plans/${encodeURIComponent(planId)}/details`);
    }
    /**
     ...
     */
    async listGoals(planId) {
        return this.graph.getAll(`https://graph.microsoft.com/beta/planner/plans/${encodeURIComponent(planId)}/goals`);
    }
    /** Category key -> label map for a plan, used to translate task labels. */
    async getCategoryDescriptions(planId) {
        try {
    ...
    async listBuckets(planId) {
        return this.graph.getAll(`/planner/plans/${encodeURIComponent(planId)}/buckets`);
    }
    async getBucket(bucketId) {
        return this.graph.getWithEtag(`/planner/buckets/${encodeURIComponent(bucketId)}`);
    }
    async createBucket(planId, name, orderHint = " !") {
        const response = await this.graph.request("/planner/buckets", {
            method: "POST",
            body: { name, planId, orderHint },
        });
        return requireBody(response.body, "bucket");
    }
    async renameBucket(bucketId, name) {
        const fresh = await this.getBucket(bucketId); // read immediately before write
        const response = await this.graph.request(`/planner/buckets/${encodeURIComponent(bucketId)}`, {
            method: "PATCH",
            body: { name },
    ...
    /* ------------------------------------------------------------------ tasks */
    async listTasksByPlan(planId) {
        return this.graph.getAll(`/planner/plans/${encodeURIComponent(planId)}/tasks`);
    }
    async listTasksByBucket(bucketId) {
        return this.graph.getAll(`/planner/buckets/${encodeURIComponent(bucketId)}/tasks`);
    }
    async listMyTasks() {
        return this.graph.getAll("/me/planner/tasks");
    }
    /** Fresh task + its own ETag. Never reuse this ETag for /details. */
    async readTask(taskId) {
        return this.graph.getWithEtag(`/planner/tasks/${encodeURIComponent(taskId)}`);
    }
    /** Fresh task details + their own, DIFFERENT ETag. Never reuse for the task. */
    async readTaskDetails(taskId) {
        return this.graph.getWithEtag(`/planner/tasks/${encodeURIComponent(taskId)}/details`);
    }
    async createTask(body) {
        const response = await this.graph.request("/planner/tasks", {
            method: "POST",
            body,
        });
    ...
    async updateTask(taskId, patch) {
        ...
        const response = await this.graph.request(`/planner/tasks/${encodeURIComponent(taskId)}`, {
            method: "PATCH",
            body: patch,
        ...
        return response.body ?? { ...fresh.value, ...patch };
    }
    /**
     ...
     */
    async getTaskCard(taskId, options = {}) {
    ...
    async buildCard(task, taskEtag, details, detailsEtag, resolveNames) {
        const notes = [];
        ...
        if (task.conversationThreadId) {
            notes.push("This card has comments. Planner comments are Microsoft 365 Group conversation posts reached through conversationThreadId — the Planner API does not expose them and this server does not read or write them.");
        }
        return {
            id: task.id,
            title: task.title,
...
function requireEtag(etag, resource) {
    if (!etag) {
        throw new M365Error("PLANNER_MISSING_ETAG", `Graph returned no @odata.etag for this ${resource}, so the write cannot be made safely.`, `This usually means the read was served from an unexpected endpoint. Re-read the ${resource} and retry; do not retry without an ETag.`);
    }
    return etag;
}
function requireBody(body, what) {
    if (body === undefined) {
        throw new M365Error("PLANNER_EMPTY_RESPONSE", `Graph accepted the ${what} but returned no body.`, `Re-read the ${what} to confirm what was created.`);
    }
    return body;
}
