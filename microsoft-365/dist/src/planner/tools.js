...
import { buildCategoryPatch, describePriority, describeProgress, toPercentComplete, toPlannerDate, toPriority, } from "./semantics.js";
const progressInput = z
    .union([z.number(), z.string()])
    .describe('Planner progress. 0 / 50 / 100, or "Not started" / "In progress" / "Completed". Planner has exactly three states — intermediate percentages are not representable.');
...
const assigneeInput = z
    .array(z.string())
    .describe('Assignees as email/UPN, display name, object GUID, or "me". Emails are resolved to object IDs automatically — Planner itself only accepts GUIDs.');
...
export function registerPlannerTools(server, ctx) {
    const { planner, users } = ctx;
    /* ------------------------------------------------------------------ plans */
    server.registerTool("planner_list_plans", {
        title: "List Planner plans",
        description: "List Planner plans. With groupId, lists the plans in that Microsoft 365 group or Team. Without it, lists the plans visible to the signed-in user plus the groups they belong to, so you can find a groupId to drill into.",
        inputSchema: {
            groupId: z
                .string()
                .optional()
    ...
    }, async (args) => runTool("planner_list_plans", async () => {
        if (args.groupId) {
        ...
        return {
            ...
            note: "Plans shared with the user only appear here once they are in the user's Planner hub. If a plan is missing, list it by its groupId.",
        };
    }));
    server.registerTool("planner_get_plan", {
        title: "Get a Planner plan",
        description: "Get a plan with its details, including categoryDescriptions — the human labels behind category1..category25. Task labels are meaningless without these.",
        inputSchema: { planId: z.string().describe("Planner plan ID.") },
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("planner_get_plan", async () => {
    ...
    server.registerTool("planner_create_bucket", {
    ...
    }, async (args) => runTool("planner_create_bucket", async () => {
    ...
    server.registerTool("planner_rename_bucket", {
        title: "Rename a Planner bucket",
        description: "Rename a bucket. The bucket is re-read immediately before the write so a current ETag is used.",
        inputSchema: {
            bucketId: z.string().describe("Planner bucket ID."),
            name: z.string().describe("New bucket name."),
        },
    }, async (args) => runTool("planner_rename_bucket", async () => {
        const bucket = await planner.renameBucket(args.bucketId, args.name);
        return { id: bucket.id, name: bucket.name, planId: bucket.planId };
    }));
    server.registerTool("planner_delete_bucket", {
        title: "Delete a Planner bucket",
        description: "Delete a bucket. Tasks in the bucket are deleted with it — this is not recoverable through the API.",
        inputSchema: {
            bucketId: z.string().describe("Planner bucket ID."),
            confirm: z
                .literal(true)
    ...
    }, async (args) => runTool("planner_delete_bucket", async () => {
    ...
    /* ------------------------------------------------------------------ tasks */
    server.registerTool("planner_list_tasks", {
        title: "List Planner tasks",
        description: "List tasks in a plan or a bucket (or the signed-in user's assigned tasks when neither is given). Returns a summary per task; use planner_get_task for the full card.",
        inputSchema: {
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("planner_list_tasks", async () => {
        let tasks;
        let scope;
        if (args.bucketId) {
        ...
        const filtered = args.includeCompleted === false ? tasks.filter((t) => (t.percentComplete ?? 0) < 100) : tasks;
        return { scope, count: filtered.length, tasks: filtered.map(summariseTask) };
    }));
    server.registerTool("planner_get_task", {
        title: "Get a Planner task (full card)",
        description: "Get one task as a complete card: title, progress, dates, priority, assignees (resolved to names), bucket, labels (resolved to their plan-level names), description, checklist and references. Task and task details are two separate Graph resources; this merges both so one call shows the whole card.",
        inputSchema: {
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("planner_get_task", async () => planner.getTaskCard(args.taskId, { resolveNames: args.resolveNames ?? true })));
    server.registerTool("planner_create_task", {
        title: "Create a Planner task",
        description: "Create a task. Description and checklist are stored on the task DETAILS resource, so supplying them performs a create followed by a details PATCH using the details' own fresh ETag.",
        inputSchema: {
            planId: z.string().describe("Planner plan ID."),
    ...
    }, async (args) => runTool("planner_create_task", async () => {
        const body = { planId: args.planId, title: args.title };
        if (args.bucketId)
            body["bucketId"] = args.bucketId;
        if (args.progress !== undefined)
            body["percentComplete"] = toPercentComplete(args.progress);
        if (args.priority !== undefined)
            body["priority"] = toPriority(args.priority);
        ...
        if (args.description !== undefined || (args.checklist && args.checklist.length > 0)) {
            const checklistOps = (args.checklist ?? []).map((title) => ({
                action: "add",
                title,
            }));
    ...
    server.registerTool("planner_update_task", {
        title: "Update a Planner task",
        description: "Update fields on the TASK resource: title, progress, dates, priority, bucket, labels and assignments. The task is re-read immediately before the write and its own fresh ETag is used — never the details ETag, which is a different resource. Description and checklist live on task details; use planner_update_task_details for those.",
        inputSchema: {
            taskId: z.string().describe("Planner task ID."),
            ...
            labels: z
                .record(z.boolean())
                .optional()
                .describe('Label changes, keyed by plan label name or category key: {"Blocked": true, "category3": false}. Only the keys you send change.'),
    ...
    }, async (args) => runTool("planner_update_task", async () => {
        const patch = {};
        if (args.title !== undefined)
            patch["title"] = args.title;
        if (args.progress !== undefined)
            patch["percentComplete"] = toPercentComplete(args.progress);
        if (args.priority !== undefined)
            patch["priority"] = toPriority(args.priority);
        const due = toPlannerDate(args.dueDateTime);
        if (due !== undefined)
            patch["dueDateTime"] = due;
        const start = toPlannerDate(args.startDateTime);
        if (start !== undefined)
            patch["startDateTime"] = start;
        if (args.bucketId !== undefined)
            patch["bucketId"] = args.bucketId;
        const wantsAssignmentChange = args.addAssignees !== undefined ||
    ...
    server.registerTool("planner_update_task_details", {
    ...
    }, async (args) => runTool("planner_update_task_details", async () => {
        const checklist = [];
        for (const item of args.addChecklistItems ?? []) {
            checklist.push(typeof item === "string"
                ? { action: "add", title: item }
                : { action: "add", title: item.title, isChecked: item.isChecked ?? false });
        }
        for (const item of args.removeChecklistItems ?? [])
            checklist.push(selector("remove", item));
        for (const item of args.checkChecklistItems ?? [])
            checklist.push(selector("check", item));
        for (const item of args.uncheckChecklistItems ?? [])
            checklist.push(selector("uncheck", item));
        for (const item of args.toggleChecklistItems ?? [])
            checklist.push(selector("toggle", item));
        ...
        const references = [
            ...
            ...(args.removeReferences ?? []).map((url) => ({ action: "remove", url })),
        ];
    ...
    server.registerTool("planner_delete_task", {
        title: "Delete a Planner task",
        description: "Delete a task. Not recoverable through the API. The task is re-read immediately before the delete so a current ETag is used.",
        inputSchema: {
            taskId: z.string().describe("Planner task ID."),
            confirm: z.literal(true).describe("Must be true. Deletion is permanent."),
        },
        annotations: { destructiveHint: true },
    }, async (args) => runTool("planner_delete_task", async () => {
        if (args.confirm !== true) {
    ...
    /* ----------------------------------------------------------------- goals */
    server.registerTool("planner_list_goals", {
        title: "List a plan's goals",
        description: "List the high-level goals set on a plan (Planner's Goals tab) — each optionally linked to specific " +
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("planner_list_goals", async () => {
        const goals = await planner.listGoals(args.planId);
        return { planId: args.planId, count: goals.length, goals };
    }));
    /* -------------------------------------------------------------- directory */
    server.registerTool("m365_resolve_user", {
        title: "Resolve a person to a directory GUID",
        description: "Resolve an email, UPN or display name to the object GUID Planner assignments require. Planner silently accepts nothing but GUIDs, so this is how you turn 'assign it to Dana' into something that works.",
        inputSchema: {
            identifier: z
                .string()
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("m365_resolve_user", async () => {
        if (args.search) {
...
function selector(action, value) {
    return { action, ...selectorFields(value) };
}
...
async function buildAssignments(ctx, identifiers) {
 ...
 */
async function buildAssignmentDelta(ctx, current, args) {
    ...
    for (const user of await ctx.users.resolveMany(args.addAssignees ?? []))
        assign(user.id);
    for (const user of await ctx.users.resolveMany(args.removeAssignees ?? []))
        unassign(user.id);
    return patch;
}
