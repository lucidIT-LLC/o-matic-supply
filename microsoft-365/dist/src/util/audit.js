...
export function initAudit(path) {
...
export function setAuditActor(upn) {
    actor = upn;
}
/** Reduce arguments to something safe to keep on disk. */
export function safeMeta(args) {
    const out = {};
...
export function auditToolCall(entry) {
    ...
    if (entry.error)
        record.error = redact(entry.error).slice(0, 240);
...
export function verifyAudit(path) {
