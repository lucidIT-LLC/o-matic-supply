...
export function registerOneNoteTools(server, ctx) {
    const { onenote } = ctx;
    server.registerTool("onenote_list_notebooks", {
    ...
    server.registerTool("onenote_search", {
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("onenote_search", async () => {
        const pages = await onenote.searchPages(args.query, resolveScope(args.scope), args.limit ?? 25);
        return { count: pages.length, pages };
    }));
    server.registerTool("onenote_get_page_content", {
        title: "Get OneNote page content",
        description: "Get a page's full HTML content exactly as OneNote stores it — headings, colors, tables, images and " +
            "attachments included, not a plain-text summary.",
        inputSchema: { pageId: z.string().describe("Page id."), scope: scopeInput },
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("onenote_get_page_content", async () => {
        const html = await onenote.getPageContent(args.pageId, resolveScope(args.scope));
        return { pageId: args.pageId, html };
    }));
    server.registerTool("onenote_create_page", {
        title: "Create a OneNote page",
        description: "Create a new page in a section from real HTML — inline CSS for colors and styling, tables, headings, " +
        ...
        inputSchema: {
            ...
            html: z
                .string()
                .describe("The page BODY as HTML — headings, paragraphs, tables, colored/styled text via inline style=\"...\", " +
    ...
    }, async (args) => runTool("onenote_create_page", async () => {
    ...
    server.registerTool("onenote_append_to_page", {
        title: "Append content to a OneNote page",
        description: "Append HTML content to the end of an existing page's body. Additive only — this cannot remove or " +
    ...
    }, async (args) => runTool("onenote_append_to_page", async () => {
