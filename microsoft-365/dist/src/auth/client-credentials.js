...
export class ClientCredentialsAuthProvider {
    ...
    static scope = DEFAULT_APP_ONLY_SCOPE;
    config;
    store;
    ...
    async getAccessToken() {
        throw new NotImplementedError("App-only (client credentials) authentication", `Set OMATIC_M365_AUTH_MODE=device-code to use the implemented delegated flow. ` +
    ...
    async status() {
        return {
            ...
            detail: "App-only authentication is declared but not implemented. No Graph call will succeed in this mode.",
        };
    }
    async signOut() {
