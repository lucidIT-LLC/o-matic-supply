...
const siteInput = z
    .string()
    .describe('SharePoint site: a full URL ("https://contoso.sharepoint.com/sites/ops"), a "hostname:/sites/path" pair, a Graph site id, or "root".');
export function registerSharePointTools(server, ctx) {
    const { sharepoint } = ctx;
    server.registerTool("sharepoint_list_lists", {
    ...
    }, async (args) => runTool("sharepoint_list_lists", async () => {
    ...
    server.registerTool("sharepoint_list_items", {
        title: "List SharePoint list items",
        description: "List items in a SharePoint list with their field values. Field keys are INTERNAL column names, which often differ from the display names; use includeColumns to see the mapping before writing.",
        inputSchema: {
            site: siteInput,
        ...
        annotations: { readOnlyHint: true },
    }, async (args) => runTool("sharepoint_list_items", async () => {
        ...
        return {
            ...
            ...(columns ? { columns } : {}),
            note: "A $filter on a non-indexed column returns an error rather than a slow result; add the column to the list's indexes in SharePoint if that happens.",
        };
    }));
    server.registerTool("sharepoint_create_item", {
        title: "Create a SharePoint list item",
        description: "Create an item in a SharePoint list. `fields` is keyed by INTERNAL column name (Title, Status, AssignedToLookupId, ...), not display name.",
        inputSchema: {
            site: siteInput,
            listId: z.string().describe("List id or list name."),
            fields: z.record(z.unknown()).describe("Field values keyed by internal column name."),
        },
    }, async (args) => runTool("sharepoint_create_item", async () => {
    ...
    server.registerTool("sharepoint_update_item", {
        title: "Update a SharePoint list item",
        description: "Update field values on a list item. Only the fields supplied are changed; SharePoint merges rather than replaces. Note this is last-writer-wins — unlike Planner, there is no ETag check.",
        inputSchema: {
            site: siteInput,
            listId: z.string().describe("List id or list name."),
    ...
    }, async (args) => runTool("sharepoint_update_item", async () => {
